from passlib.hash import pbkdf2_sha256

import uuid


class MysqlBackend(object):
    USER_SCHEMA = """
        CREATE TABLE auth_users (
            _id INT AUTO_INCREMENT PRIMARY KEY,
            uuid CHAR(32) NOT NULL UNIQUE,
            username VARCHAR(128) NOT NULL UNIQUE,
            email VARCHAR(256) UNIQUE,
            password VARCHAR(1024),
            is_active BOOL,
            created_on DATETIME DEFAULT CURRENT_TIMESTAMP,
            last_login_on DATETIME NULL
        )
    """

    def __init__(self, config):
        self.config = config

    def init(self):
        self.connection = self.config.modules.database.instance.get()
        self.session = self.config.modules.session

        # Try and create the schema
        try:
            cursor = self.connection.cursor()
            cursor.execute(self.USER_SCHEMA)
            self.connection.commit()
        except Exception as _:
            pass
        finally:
            cursor.close()

    def user_create(self, username, password, active=False):
        active = 1 if active else 0
        password = pbkdf2_sha256.hash(password)

        query = """
            INSERT INTO
                `auth_users` (`uuid`, `username`, `password`, `is_active`)
            VALUES
                (%s, %s, %s, %s)
        """
        with self.connection.cursor() as cursor:
            cursor.execute(query, (
                uuid.uuid4().hex,
                username,
                password,
                active
            ))
            self.connection.commit()

    def user_check_exists(self, username):
        query = """
            SELECT
                COUNT(`username`)
            FROM
                auth_users
            WHERE
                `username` = %s
        """
        with self.connection.cursor() as cursor:
            cursor.execute(query, (username, ))
            result = self.fetchone()

    def user_check_login(self, username, password):
        # password = pbkdf2_sha256.hash(password)

        query = """
            SELECT
                `username`, `password`
            FROM
                auth_users
            WHERE
                `username` = %s
        """
        with self.connection.cursor() as cursor:
            cursor.execute(query, (
                username,

            ))
            result = cursor.fetchone()
            return pbkdf2_sha256.verify(password, result['password'])

    def user_activate(self, username, code):
        query = """
            UPDATE
                auth_users
            SET
                active = 1
            WHERE
                `username` = %s
        """
        with self.connection.cursor() as cursor:
            cursor.execute(query, (username, ))
            self.connection.commit()

    def user_deactivate(self, username):
        query = """
            UPDATE
                auth_users
            SET
                active = 0
            WHERE
                `username` = %s
        """
        with self.connection.cursor() as cursor:
            cursor.execute(query, (username, ))
            self.connection.commit()

    def user_change_password(self, username, new_password):
        password = pbkdf2_sha256.hash(new_password)
        query = """
            UPDATE
                auth_users
            SET
                password = %s
            WHERE
                `username` = %s
        """
        with self.connection.cursor() as cursor:
            cursor.execute(query, (password, username, ))
            self.connection.commit()

    def user_update(self, username, **fields):
        pass
